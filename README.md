...
| `network.allowedServices` | list of strings | Service shortcuts that expand to curated host allowlists. Available: `anthropic`, `cachix`, `dockerhub`, `github`, `gitlab`, `google`, `nixpkgs`, `npm`, `openai`, `pypi`. Merged with `network.allowedHosts`. Inbound traffic is not filtered. |
